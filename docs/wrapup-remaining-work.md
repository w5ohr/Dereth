...
| ✅ **Loot item-spells + mana + spellcraft** | ~~Mutated cantrips (Minor/Major/Epic/Legendary), mana pools on gear, Spellcraft stat.~~ DONE — built independently by BOTH agents and reconciled in the #127 merge: **procedural cantrips** ride main's Tier-2C system (`rollWield` rolls a Minor/Major/Epic/Legendary jewelry-style bonus with its own `imana` pool, drains 0.35/s worn, Mana Stone refills); **the 1,249 catalog items carrying retail spell ids** apply live worn effects via #127's `itemSpellFx` (Blood Drinker/Heart Seeker/Swift Killer/Defender/Protections/attributes/aptitudes → their channels; unmatched names stay as lore) with `spellcraft`-driven Arcane Lore gates and slower-draining `itemMana` pools. Mana stones fill rolled-cantrip pools, then catalog pools, then the focus, then your reserve. | L | auth-gaps I2 |
