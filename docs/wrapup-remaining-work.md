...
| ✅ **Training Academy tutorial dungeon** | ~~Build the ToD Academy interior + courtyard (skippable).~~ DONE (PR #127) — new arrivals now wake **inside** the Society's hall, a full authored interior instance (Great Hall → open Courtyard → Workshop & Library wings → Gatehouse) built on the dungeon shell; the complete existing station chain runs indoors (Greeter takes the Calling Stone → Samuel's armor → Training Master + Sparring Golem in the courtyard → Foreman/wasp → Blacksmith/thrungus → Researcher/oil → Sentry/olthoi → coat + Facility Hub Portal Gem → your chosen town), practice targets spawn in the courtyard, **Jonathan's instant skip** works (+11k XP + kit), the gate arch refuses until the Sentry releases you, and the Greeter speaks her **true retail tutorial lines from `acdialogue.json`** ("Follow the GLOWING ARROWS…"). Mid-training saves re-enter the hall on load; the outdoor grounds remain for old saves. Verified e2e both paths. *(Known: staff leak on in-hall death — playtest #23.)* | L | ac-newplayer B1, ac-remaining-gaps |
...
| ✅ **Loot item-spells + mana + spellcraft** | ~~Mutated cantrips (Minor/Major/Epic/Legendary), mana pools on gear, Spellcraft stat.~~ DONE — the 1,249 catalog items carrying retail spell ids now apply live worn effects (`itemSpellFx` maps Blood Drinker/Heart Seeker/Swift Killer/Defender/Protections/attribute buffs/aptitudes to their channels; unmatched names stay as lore); procedural gear rolls **Minor/Major/Epic/Legendary aptitude cantrips** (tier-scaled, count toward the Arcane Lore gate); **spellcraft** rolled from tier+workmanship drives the Arcane Lore requirement; spell-bearing gear carries a **mana pool** that drains while worn (dormant at 0; mana stones recharge worn gear first and rekindle it). Tooltips show cantrips/spells/spellcraft/pool. Verified live end-to-end. | L | auth-gaps I2 |
...
| ✅ **Aetheria slot/level/surge system** | ~~Slots unlock L75/150/225, leveled aetheria, set bonus + surge procs (currently 3 flat gems).~~ DONE (PR #124, reconciled with PR #127) — three colour slots (blue L75 / yellow L150 / red L225, shown as paperdoll tiles); aetheria drop from tier-4+ foes once the first slot is open, each with a **level 1–5** (higher rarer), a **set sigil** passive (Growth/Vigor/Defense/Destruction/Fury) and a **surge** that flares in combat (Destruction/Fury on your strikes; Protection/Regeneration/Affliction when struck; one at a time, 9s CD). Both agents built this independently — main's (PR #124) implementation was kept in the merge; the paperdoll slot tiles from #127 ride on top. | L | auth-gaps I5 |
| 🟡 **Item models for shields/clothing** | **Clothing/armour is DONE** — `ac_item_models.py` already bakes them (297 clothing/armour meshes in `acitemmodels/index.json`: caps/coats/gauntlets/…), so clothing & armour drops already show real meshes via `AC_ITEM_MODELS`. **Only SHIELDS remain** — shield Setups weren't captured (offhand meshes). Focused follow-up: extract the shield Setup class and bake it (mind pack size); low priority (offhand cosmetic). | S–M | REMAINING-WORK |
...
| ✅ **Authored landmarks + terrain barriers** | ~~Landblock heightmap + named mountains that gate travel (currently procedural noise).~~ DONE — the **[needs A1 data]** flag was stale: `acmap.png` real elevations already drive the terrain. Added **nine named landmarks** (Mount Esper, Marescent Plateau, Obsidian Plains, Linvak Mountains, South Osteth Peak, Lakes Nemuel/Thrasyl/Scimitar, Mount Alphus) anchored to **real retail portal endpoints** in `acworld.json` — peaks re-seat on the true data summit; arrival toasts + lore + world-map labels. **Terrain barriers**: slopes steeper than ~60° stop travel (contour-slide first; roads and town plateaus exempt) — verified mountain flanks gate (3.3–112 rise/run) while farmland (≤0.44) never trips. | L | auth-gaps W12 |
