...
| 20 | A | **TestSystemA TSA-1 — duplicate `craftResultItem` (merge artifact)** | index.html declares it TWICE (≈14355 and ≈14528) with different bodies; hoisting silently runs the newer (dye-pot-aware) one, the older is dead and invites divergence. Fix: delete the first body. *(TestSystemA run 2)* | S | open |
