...
| 9 | **Geometry-driven *town* renderer** | Largely closed — dungeons render real EnvCell meshes and towns already stream real AC building meshes (`tbBuildMesh`). Only true EnvCell *town-layout* geometry remains, and it overlaps the A1 lane. **⚠ Note:** `ace_building_export.py` has the SAME `const COORD=(\d+)` parse bug (reads 80 not 240), so `actowns.json` building **offsets are baked at ⅓ scale** — town cores are compact relative to the 1:1 world. Unlike the world-structures case this isn't *broken* (towns aren't in the ocean; dense cores are arguably authentic), but it's an inconsistency to resolve on a real-GPU pass with the town lane, not headless. *(ac-data-extraction-roadmap)* | follow-up | blocked |
