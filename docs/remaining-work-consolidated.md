...
| 5 | C | **`path_blocked` interior-packing residual** | A handful of tightly-packed town buildings whose entry centreline a neighbour grazes (door still usable at an angle). `door_offmarker` & `floor_poke` already resolved. *(building-entry-audit)* | S | done |
| 6 | C | **`0x21` StringTable UI-layout decode** *(low value)* | 101 retail 800×600 panel-layout defs identified but not decoded; only useful for pixel-perfect panel placement. *(three-agent-dat-update)* | follow-up | done |
